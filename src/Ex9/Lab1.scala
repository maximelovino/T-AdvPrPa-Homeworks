...
abstract class Drawable(var center: Point = null, var transparency: Double = 0) {
  ...
  def -->(to: Drawable)(implicit d: Drawing): Unit = d += new Arrow(this, to)
}
...
abstract class Effect(val duration: Int) {
  ...
  def ==>(effect: Effect) = new InOrderEffect(this, effect)

  def ||(effect: Effect) = new TogetherEffect(this, effect)

  def unary_-() = new ReverseEffect(this)
...
class EffectInt(n: Int) {
  def times(fx: Effect): Effect = fx times n
}
...
class UpdateEffect(duration: Int, f: Double => Unit) extends Effect(duration) {
  override def act(t: Int): Unit = f(completion(t))
}

object UpdateEffect {
  def update(duration: Int)(f: Double => Unit) = new UpdateEffect(duration, f)
}

object Lab1 extends App {

  implicit def intToEffectInt(n: Int) = new EffectInt(n)

  val f = new JFrame
  implicit val d: Drawing = new Drawing
  f.add(d)
  f.pack()
  f.setVisible(true)
  ...
  val c1Move = new MoveEffect(c1, new Point(400, 200), 2000)
  val c2Move = new MoveEffect(c2, new Point(200, 200), 6000)
  val c2Hide = new HideEffect(c2, 3000)
  val c2Grow = UpdateEffect.update(2000) {
    t => c2.radius = 30 + 20 * t
  }
  val ordered = c1Move ==> (c2Move || (c2Hide ==> -c2Hide) ==> (3 times c2Grow))
  d.start(ordered)
}