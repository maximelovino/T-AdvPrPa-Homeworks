...
object NumericalIntegration extends App {
  ...
  def integrate(a: Double, b: Double, nIntervals: Int, f: Double => Double): Double = {
    ...
    range.foldLeft(0.0)((acc, current) => acc + trapezArea(f(current), f(current + intervalSize), intervalSize))
  }

  def integrateLazy(a: Double, b: Double, nIntervals: Int, f: Double => Double): Double = {
    val intervalSize = (b - a) / nIntervals
  ...
  def integrateLazyPar(a: Double, b: Double, nIntervals: Int, f: Double => Double): Double = {
    val intervalSize = (b - a) / nIntervals
  ...
  val INTERVALS = Math.pow(20, 6).toInt
